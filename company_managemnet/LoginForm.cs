    ...
    public class LoginForm : Form
    {
        ...
        private void OnLoginClick(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                PersonModel person    = personBLL.Login(email, password);
                ...
                HeadOfDeptBLL hodBLL = new HeadOfDeptBLL();
                EmployeeBLL   empBLL = new EmployeeBLL();

                if (hodBLL.IsHOD(person.ID))
                {
                    HeadOfDeptModel hod = hodBLL.GetByID(person.ID);
                    HODDashboard dashboard = new HODDashboard(hod);
                    this.Hide();
                    dashboard.FormClosed += new FormClosedEventHandler(OnChildClosed);
                    dashboard.Show();
                }
                else if (empBLL.IsEmployee(person.ID))
                {
                    EmployeeModel emp = empBLL.GetByID(person.ID);
